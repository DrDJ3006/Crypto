...
class AES:
    ...
    def encrypt(self, key, plain_text, enc_mod="ecb", salt=b'', info=b'', initial_vector=None):
        if len(key) != self.key_bytes_size:
            key = hkdf(key, self.key_bytes_size,salt=salt,info=info)
        data = add_pkcs7_padding(plain_text)
        if enc_mod == "ecb":
            cipher_text = b''
            for i in range(0, len(data), 16):
                encrypted_block = self.encrypt_block(key, data[i:i+16])
    ...
    def decrypt(self, key, cipher_text, enc_mod="ecb", salt=b'', info=b'' ):
        if len(key) != self.key_bytes_size:
            key = key = hkdf(key, self.key_bytes_size,salt=salt,info=info)
    ...
    def encrypt_file(self, key, source_file, dest_file, enc_mod="cbc", salt=b'', info=b'', initial_vector=None):
        if len(key) != self.key_bytes_size:
            key = hkdf(key, self.key_bytes_size,salt=salt,info=info)
        if enc_mod == "cbc":
    ...
    def decrypt_file(self, key, source_file, dest_file, salt=b'', info=b'',  enc_mod="cbc"):
        if len(key) != self.key_bytes_size:
            key = hkdf(key, self.key_bytes_size,salt=salt,info=info)
        ...
        if enc_mod == "cbc":
            ...
            with open(dest_file, mode="wb") as df:
                for i in range(0, len(cipher_text), 16):
                    ...
                    # Check if accumulated data has reached 1KB or if it's the final block
                    if len(decrypted_data_accumulator) >= 1024 or i + 16 >= len(cipher_text):
